...
export type Rx = {
    ...
    next_refill_date?: string | null; // ISO date
    notes?: string | null;
    created_at: string;
    updated_at: string;
};
...
export async function updatePrescription(id: string, values: Partial<Rx>) {
    const { data, error } = await supabase.from("prescriptions").update(values)
...
export async function deletePrescription(id: string) {
    const { error } = await supabase.from("prescriptions").delete().eq(
        "id",
        id,
    );
    if (error) throw error;
